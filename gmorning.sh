...
function get_yahoo_finance_active_tickers()
{
    ...
    active_tickers_value=$(echo "$yahoo_finance" | grep -Eo "class=\"symbol.+class=.+" | grep -Eo ">[0-9,\.]+" | cut -c 2- | sed "s/[,]//g") 
    active_tickers_perct_change=$(echo "$yahoo_finance" | grep -Eo "class=\"txt-positive [a-z0-9\-]+\">\([%0-9\.\+\-]+|class=\"txt-negative [a-z0-9\-]+\">\([%0-9\.\+\-]+" | grep -Eo "(\+[0-9\.\%]+|-\+[0-9\.\%]+)")
